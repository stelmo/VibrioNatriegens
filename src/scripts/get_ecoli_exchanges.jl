...
get_kegg_compound_id(mid) = begin
    k = get(model.metabolites[model.metabolite_index[mid]]["annotation"], "kegg.compound", nothing)
...
d = Dict(
    exrid => get_kegg_compound_id(exrid) for exrid in exchange_rids if !isnothing(get_kegg_compound_id(exrid))
)
