...
function normalize_reaction_stoichiometry!(model, rid)
    # make reaction stoichiometry 1 based, easier flux fitting interpretation

    rxn_st = model.reactions[rid].stoichiometry
    sts = unique(abs.(collect(values(rxn_st))))
    
    if all([rem(x, 1) for x in sts] .== 0)
        sts = Int.(sts) 
    else 
        return nothing
    end
...
function normalize_stoichiometry!(model)
    for rid in keys(model.reactions)
        normalize_reaction_stoichiometry!(model, rid)
    end
end
