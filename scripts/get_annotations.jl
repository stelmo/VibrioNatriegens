...
FASTAReader(open(joinpath("data", "genome", "proteome.fasta"))) do reader
    for record in reader
        pid = string(
            first(split(string(last(split(description(record), "protein_id="))), "]")),
        )
        length(pid) <= 20 && begin
...
seq_df = DataFrame(
    ProteinAccession = collect(keys(gene_seqs)),
    Sequence = collect(values(gene_seqs)),
)
...
molar_masses = DataFrame(CSV.File(joinpath("data", "genome", "vnat_molar_masses.csv")))
...
inchi_df = DataFrame(CSV.File(joinpath("data", "chebi", "chebiId_inchi.tsv")))
inchi_dict = Dict(zip("CHEBI:" .* string.(inchi_df.CHEBI_ID), inchi_df.InChI))

chemical_df = DataFrame(CSV.File(joinpath("data", "chebi", "chemical_data.tsv")))
@subset!(chemical_df, :SOURCE .== "ChEBI", :TYPE .== "MASS")
chemical_dict = Dict(
    "CHEBI:" * string(first(gdf.COMPOUND_ID)) =>
        parse(Float64, first(gdf.CHEMICAL_DATA)) for
    gdf in groupby(chemical_df, :COMPOUND_ID)
)
...
for k in keys(inchi_dict)
    push!(
        ...
        (
            k,
            get(kegg_compound, k, missing),
            get(chemical_dict, k, missing),
            get(inchi_dict, k, missing),
            get(names_dict, k, missing),
        ),
    )
end
df
